...
Use the CLI-first state surface (`omq state ... --json`) for ultrawork lifecycle state. If explicit MCP compatibility tools are already available, equivalent `omq_state` calls are optional compatibility, not the default.

- **On start**:
  `omq state write --input '{"mode":"ultrawork","active":true,"reinforcement_count":1,"started_at":"<now>"}' --json`
- **On each reinforcement/loop step**:
  `omq state write --input '{"mode":"ultrawork","reinforcement_count":<current>}' --json`
- **On completion**:
  `omq state write --input '{"mode":"ultrawork","active":false}' --json`
- **On cancellation/cleanup**:
  run `$cancel` (which should call `omq state clear --input '{"mode":"ultrawork"}' --json`)
