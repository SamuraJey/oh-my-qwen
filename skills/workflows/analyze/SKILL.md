...
- the request is purely about running the OMQ tmux team runtime — use `$team` only when OMQ runtime is active
...
- Use **`$team` only when OMQ runtime is active** and durable tmux-based coordination is actually needed.
