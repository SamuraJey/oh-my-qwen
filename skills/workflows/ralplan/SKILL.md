...
Ralplan is not complete, skippable, or ready for execution merely because `.omq/plans/prd-*.md` and `.omq/plans/test-spec-*.md` exist. Those files are planning artifacts, not consensus evidence.
...
- `$ultragoal` — **default goal-mode follow-up** for implementation or general goal-oriented follow-up plans that should become durable Codex/OMQ goals with sequential completion tracking.
...
4. If ambiguity remains high, gather brownfield facts first. `omq explore` is deprecated; use normal repository inspection tools/subagents for simple read-only repository lookups and `omq sparkshell` only for explicit shell-native read-only evidence. Then run `$deep-interview --quick <task>` before continuing.
