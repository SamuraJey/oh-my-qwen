...
- **ultragoal**: Durable goal-mode execution with `.omq/ultragoal` ledgers. Launch `$team` only from inside an Ultragoal story when parallel lanes are warranted.
...
- **On start**: `omq state write --input '{"mode":"pipeline","active":true,"current_phase":"stage:ralplan"}' --json`
- **On stage transitions**: `omq state write --input '{"mode":"pipeline","current_phase":"stage:<name>"}' --json`
- **On completion**: `omq state write --input '{"mode":"pipeline","active":false,"current_phase":"complete"}' --json`
