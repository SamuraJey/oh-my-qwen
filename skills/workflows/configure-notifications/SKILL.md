...
```bash
CONFIG_FILE="$HOME/.codex/.omq-config.json"
...
- For Discord bot replies, an authorized operator can reply with exact-match `status` to a tracked OMQ notification to receive a bounded read-only session summary. This is a reply-thread-scoped status probe, not a general remote control surface.
...
- Config path (`~/.codex/.omq-config.json`)
