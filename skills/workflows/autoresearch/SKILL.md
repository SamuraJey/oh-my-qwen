...
Autoresearch is the skill-first replacement for the deprecated `omq autoresearch` command.
...
## Do not use when
- You want the old `omq autoresearch` command surface (hard-deprecated)
...
2. **Persist mode state** in `.omq/state/.../autoresearch-state.json` including:
   - `validation_mode`
...
```json
{
  ...
  "output_artifact_path": ".omq/specs/autoresearch-demo/report.md"
}
```

## Recommended flow
1. Run `$deep-interview --autoresearch` to clarify mission + evaluator.
2. Materialize `.omq/specs/autoresearch-{slug}/mission.md`, `sandbox.md`, and `result.json`.
...
## Migration note
- `omq autoresearch` is hard-deprecated.
