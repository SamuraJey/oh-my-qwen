...
`$prometheus-strict` is a clean-room OMQ planning skill for rigorous interview-driven planning before execution.
...
- **Oracle** synthesizes the approved plan and recommends the OMQ-native handoff.
...
The skill returns a Prometheus Strict Plan with clarified requirements, resolved critique, an Oracle execution plan, a verification matrix, an optional durable artifact path under `.omq/plans/prometheus-strict/`, and clean-room credit.

## Durable Plan Artifacts

When the plan should survive handoff or review, write the final Oracle synthesis to `.omq/plans/prometheus-strict/<slug>.md` and include that path in the plan before invoking `$ultragoal` or `$team`. Inline-only plans may set the artifact path to `N/A - inline plan only`.
