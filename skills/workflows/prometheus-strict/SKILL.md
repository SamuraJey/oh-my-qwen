...
description: "[OMQ] Clean-room interview-driven planner: Metis clarifies, Momus challenges, Oracle synthesizes, then hands off to $ultragoal/$team."
argument-hint: "<goal or problem statement>"
---

# Prometheus Strict

Clean-room OMQ planning workflow inspired by the high-level OMO Prometheus concept only. This skill does not copy implementation, prompts, wording, control flow, or runtime code from OMO. It reimplements the idea under this repository's MIT-licensed skill conventions.

Credit: Inspired by OMO Prometheus (`code-yeongyu/oh-my-openagent`), reimplemented from concept under MIT.

<Purpose>
Prometheus Strict creates a rigorous plan before execution when ambiguity is still risky. It separates three planning voices: Metis clarifies requirements, Momus challenges assumptions and validation gaps, and Oracle synthesizes the handoff-ready OMQ-native plan.

The output is a planning-only artifact for `$ultragoal` and, when independent lanes are justified, `$team`. When a durable artifact is useful, store or request the final plan under `.omq/plans/prometheus-strict/`.
</Purpose>
...
<Why_This_Exists>
OMQ already has `$plan`, `$ralplan`, and `$deep-interview`. Prometheus Strict exists for a narrower case: an explicit clean-room strict-planning lane with named clarification, critique, and synthesis roles, plus a durable `.omq/plans/prometheus-strict/` handoff contract. It is not a replacement for execution workflows.
</Why_This_Exists>
...
- In attached-tmux OMQ runtime, use `omq question` as the OMQ-owned structured question surface (this is the `AskUserQuestion` equivalent for Prometheus Strict). From attached-tmux Bash/tool paths, prefix the command with `OMQ_QUESTION_RETURN_PANE=$TMUX_PANE` (or a concrete `%pane` value) so the leader-pane return target is preserved.
...
- (a) `omq question` batch: use when at least one CRITICAL question survives `<gap_triage>` and `<self_review>`. The batch is the round; the turn waits for `answers[]` before continuing.
...
Edge cases:
1. Zero-questions-but-complete-checklist → option (b) explicit handoff. Do not emit an empty `omq question` form.
...
**Pass 1 — Synthesis.** Oracle produces the final objective, scope and non-goals, accepted assumptions, resolved critique, sequenced steps or lanes, verification matrix, rollback/escalation conditions, and recommended OMQ handoff.
...
```text
$ultragoal "<Oracle plan summary or .omq/plans/prometheus-strict/<slug>.md>"
...
Prometheus Strict does not own a long-running runtime loop. If a durable planning artifact is needed, write the final plan to `.omq/plans/prometheus-strict/<slug>.md`. Draft-only or inline plans may set the artifact path to `N/A - inline plan only`.
...
If writing a durable plan file, store this markdown at `.omq/plans/prometheus-strict/<slug>.md` and reference that path in the handoff.
...
### Artifact
- Durable plan path: `.omq/plans/prometheus-strict/<slug>.md` or `N/A - inline plan only`
