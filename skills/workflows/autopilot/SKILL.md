---
name: autopilot
description: "[OMQ] Strict autonomous loop: $deep-interview -> $ralplan -> $ultragoal (+ $team if needed) -> $code-review -> $ultraqa"
---
...
3. **Phase `ultragoal`** — durable implementation + verification loop
   ...
   - Ultragoal owns durable Codex goal handoffs, `.omq/ultragoal` ledger checkpoints, implementation, tests, build/lint/typecheck evidence, cleanup, and final review gate discipline.
...
2. Reuse the latest relevant `.omq/context/{slug}-*.md` snapshot when available.
...
- Use existing hooks, `.omq/state`, `$deep-interview`, `$ralplan`, `$ultragoal`, optional `$team`, `$code-review`, `$ultraqa`, and pipeline primitives; do not invent a separate execution framework.
...
<State_Management>
Use the CLI-first state surface (`omq state ... --json`) for Autopilot lifecycle state. State must be session-aware when a session id exists. If the explicit MCP compatibility surface is already available, equivalent `omq_state` tool calls remain acceptable but are not required.
