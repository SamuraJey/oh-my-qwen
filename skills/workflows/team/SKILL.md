...
`$team` is the tmux-based parallel execution mode for OMQ. It starts real worker Codex and/or Claude CLI sessions in split panes and coordinates them through `.omq/state/team/...` files plus CLI team interop (`omq team api ...`) and state files.

This skill is operationally sensitive. Treat it as an operator workflow, not a generic prompt pattern. In Codex App or plain outside-tmux sessions, do not present `$team` / `omq team` as directly available; launch OMQ CLI from shell first, or stay on the nearest app-safe surface until the user explicitly wants the tmux runtime.
...
Use `$ultragoal` for durable leader-owned goal/ledger tracking and `$team` for parallel execution lanes. When Team is launched with an active `.omq/ultragoal/goals.json`, worker inboxes/status may include leader-owned Ultragoal context: `.omq/ultragoal/goals.json`, `.omq/ultragoal/ledger.jsonl`, the active goal id, Codex goal mode, and the `fresh_leader_get_goal_required` checkpoint policy.

Workers provide task status and verification evidence only. They do not own Ultragoal goal state, create worker ledgers, mutate `.omq/ultragoal`, auto-launch Team from Ultragoal, or perform hidden Codex goal mutation. The leader uses terminal Team evidence plus a fresh `get_goal` snapshot to run `omq ultragoal checkpoint --goal-id <id> --status complete --evidence "<team evidence mentioning .omq/ultragoal and <id>>" --codex-goal-json <fresh-get_goal-json-or-path>`.
...
OMQ_TEAM_WORKER_CLI_MAP=codex,claude omq team 2:executor "split doc/code tasks"
...
- include an explicit launch hint (`omq team N "<task>"` / `$team N "<task>"`) for the coordinated team run; mention `$ultragoal` as the default durable follow-up/ledger path; mention a later separate Ralph follow-up only when explicitly requested or genuinely needed as a fallback
...
7. Launch workers with:
   ...
   - worker CLI selected by `OMQ_TEAM_WORKER_CLI` / `OMQ_TEAM_WORKER_CLI_MAP` (`codex` or `claude`)
...
- When the leader assigns teammate roles/tasks, OMQ allocates **per-worker reasoning effort dynamically** from the resolved worker role and `agentReasoning` overrides (`low`, `medium`, `high`, `xhigh`).
...
If the leader gets a stale, lifecycle, or all-idle nudge, immediately run `omq team status <team-name>` before taking any manual intervention. Deprecated worker stall/progress nudges should not be treated as an active runtime contract.

### Deprecated worker stall/progress knobs

`OMQ_TEAM_PROGRESS_STALL_MS` and `OMQ_TEAM_WORKER_TURN_STALL_MS` are legacy compatibility/test-only names for the retired worker stall/progress nudge path. Do not recommend them as operator tuning knobs for active team runs; resolved native worker Stop, all-idle, mailbox, and stale-leader evidence are the supported leader wakeup signals.
...
- `OMQ_TEAM_WORKER_CLI_MAP`
  ...
  - Example: `OMQ_TEAM_WORKER_CLI_MAP=codex,codex,claude,claude`
...
Use the `omq team ...` CLI as the supported team-launch surface. For automation, drive the same CLI flow from scripts or supervising agents rather than relying on a separate MCP runner.

### Supported current surfaces

- **`omq team ...` CLI** — Primary method for interactive or automated team orchestration. Use this when you want direct tmux-pane visibility or a scriptable launch path.
- **Team state files** — Inspect `.omq/state/team/<team>/` when you need status, task, or mailbox evidence after launch.
...
- tmux/session cleanup: Use the documented `omq team` shutdown / cleanup flow when you need to stop worker panes or clean up an interrupted run.

### Automation example

```
1. omq team 1:executor "fix bugs"
2. omq team status <team-name>
3. omq team shutdown <team-name>
