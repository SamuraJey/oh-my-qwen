...
0. **Pre-context intake (required before planning/execution loop starts)**:
   ...
   - If request ambiguity is high, gather brownfield facts first. `omq explore` is deprecated; use normal repository inspection tools/subagents for simple read-only repository lookups and `omq sparkshell` only for explicit shell-native read-only evidence. Then run `$deep-interview --quick <task>` to close critical gaps.
...
7.5 **Mandatory Deslop Pass**:
   - After Step 7 passes, run `oh-my-qwen:ai-slop-cleaner` on **all files changed during the Ralph session**.
...
- Use `omq state write/read --input '<json>' --json` for ralph mode state persistence between iterations
...
- Prefer CLI state commands. If an explicit MCP compatibility `omq_state` call reports that its stdio transport is unavailable/closed, do **not** retry the same MCP call. Retry once through the supported CLI parity surface with the same payload, preserving `workingDirectory` and `session_id`: `omq state write --input '<json>' --json`, `omq state read --input '<json>' --json`, or `omq state clear --input '<json>' --json`. If the CLI path also fails, continue with `.omq/context` / `.omq/plans` file-backed artifacts and report the state persistence blocker.
</Tool_Usage>
...
Use the CLI-first state surface for Ralph lifecycle state (`omq state write/read/clear --input '<json>' --json`). Explicit MCP compatibility tools (`state_write`, `state_read`, `state_clear`) remain acceptable only when already enabled.

- **On start**:
  `omq state write --input '{"mode":"ralph","active":true,"iteration":1,"max_iterations":10,"current_phase":"executing","started_at":"<now>","state":{"context_snapshot_path":"<snapshot-path>"}}' --json`
- **On each iteration**:
  `omq state write --input '{"mode":"ralph","iteration":<current>,"current_phase":"executing"}' --json`
- **On verification/fix transition**:
  `omq state write --input '{"mode":"ralph","current_phase":"verifying"}' --json` or `omq state write --input '{"mode":"ralph","current_phase":"fixing"}' --json`
- **On completion** (only after the completion audit passes with real evidence):
  `omq state write --input '{"mode":"ralph","active":false,"current_phase":"complete","completed_at":"<now>","completion_audit":{"passed":true,"prompt_to_artifact_checklist":["<requirement mapped to artifact/evidence>"],"verification_evidence":["<fresh test/build/lint command and result>"]}}' --json`
- **Before the final answer**:
  ...
  5. Read the state back with `omq state read --input '{"mode":"ralph"}' --json` and verify `completion_audit.passed === true`, a non-empty checklist, and non-empty verification evidence before producing the final answer.
  6. If Codex goal mode is active, call `update_goal({status:"complete"})` only after this Ralph audit read-back succeeds.
- **On cancellation/cleanup**:
  run `$cancel` (which should call `omq state clear --input '{"mode":"ralph"}' --json`)
...
Prompt-side `$ralph` workflow activation is lighter-weight than `omq ralph --prd ...`.
It seeds Ralph workflow state and guidance, but it does not implicitly launch the
CLI entrypoint or apply the PRD startup gate. Treat `omq ralph --prd ...` as the
...
- During the compatibility window, Ralph `--prd` startup still validates machine-readable story state from `.omq/prd.json`.
- `.omq/plans/prd-{slug}.md` remains the canonical storage/documentation artifact, but it is not yet the startup validation source.
