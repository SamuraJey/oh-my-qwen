...
OMQ installs skills to `${CODEX_HOME:-~/.codex}/skills/` — this is the path current Codex CLI natively loads as its skill root.

`~/.agents/skills/` is a **historical legacy path** from an older Codex CLI release, before Codex settled on `~/.codex` as its home directory. Current Codex CLI and OMQ no longer write there.

**In a mixed OMQ + plain Codex environment:**
...
- **Interop rule**: OMQ writes only to the canonical path; archive or remove `~/.agents/skills/` once you have confirmed `${CODEX_HOME:-~/.codex}/skills/` is your active root
...
Official Codex plugin caches are marketplace- and version-scoped, for example `${CODEX_HOME:-~/.codex}/plugins/cache/$MARKETPLACE_NAME/oh-my-qwen/$VERSION/`. Local installs may use `local` as the version identifier.

```bash
# Get installed plugin cache versions across marketplaces.
# Cache shape: $PLUGIN_CACHE_ROOT/$MARKETPLACE_NAME/oh-my-qwen/$PLUGIN_VERSION/
PLUGIN_CACHE_ROOT="${CODEX_HOME:-$HOME/.codex}/plugins/cache"
CACHE_ENTRIES=$(find "$PLUGIN_CACHE_ROOT" -path "*/oh-my-qwen/*" -mindepth 3 -maxdepth 3 -type d 2>/dev/null)
...
# Get latest from npm
LATEST=$(npm view oh-my-qwen version 2>/dev/null)
echo "Latest npm: $LATEST"
```

**Diagnosis**:
- If no cache entry exists: INFO - plugin marketplace artifact not cached; this may be normal when OMQ was installed only through npm/setup
...
- Remember: plugin install/discovery is not a replacement for `npm install -g oh-my-qwen` plus `omq setup`; the packaged plugin carries plugin-scoped companion metadata for optional MCP compatibility servers and apps, with first-party MCP disabled by default, while native/runtime hooks and the rest of OMQ runtime wiring stay setup-owned
...
grep -q "oh-my-qwen Multi-Agent System" ~/.codex/AGENTS.md 2>/dev/null && echo "Has OMQ config" || echo "Missing OMQ config"
```
...
PLUGIN_CACHE_ROOT="${CODEX_HOME:-$HOME/.codex}/plugins/cache"
find "$PLUGIN_CACHE_ROOT" -path "*/oh-my-qwen/*" -mindepth 3 -maxdepth 3 -type d 2>/dev/null \
  | while IFS= read -r VERSION_DIR; do
      MARKETPLACE_NAME=$(basename "$(dirname "$(dirname "$VERSION_DIR")")")
      PLUGIN_VERSION=$(basename "$VERSION_DIR")
...
**Diagnosis**:
- If `~/.codex/agents/` exists with oh-my-qwen-related files: WARN - legacy generated agents or hand-installed role files. The Codex plugin can package reusable workflows plus plugin-scoped companion metadata for optional MCP/apps; legacy setup installs native agents, while plugin setup archives stale legacy native-agent files and keeps config/hooks current.
- If `~/.codex/commands/` exists with oh-my-qwen-related files: WARN - legacy command files from older installs. Current OMQ uses skills/workflows plus setup-managed native surfaces.
- If `${CODEX_HOME:-~/.codex}/skills/` exists with OMQ skills: OK - canonical current user skill root
...
- Any oh-my-qwen-related `.md` files in skills/
...
PLUGIN_CACHE_ROOT="${CODEX_HOME:-$HOME/.codex}/plugins/cache"
find "$PLUGIN_CACHE_ROOT" -path "*/oh-my-qwen" -type d -prune -exec rm -rf {} +
...
PLUGIN_CACHE_ROOT="${CODEX_HOME:-$HOME/.codex}/plugins/cache"
PLUGIN_CACHE_DIR="$PLUGIN_CACHE_ROOT/$MARKETPLACE_NAME/oh-my-qwen"
...
```
WebFetch(url: "https://raw.githubusercontent.com/Yeachan-Heo/oh-my-qwen/main/docs/AGENTS.md", prompt: "Return the complete raw markdown content exactly as-is")
```
