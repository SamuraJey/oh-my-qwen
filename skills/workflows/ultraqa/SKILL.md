...
5. **Stale state**: old `.omq/state` files, mismatched sessions, missing timestamps, and contradictory phase metadata.
...
Use the CLI-first state surface (`omq state ... --json`) for UltraQA lifecycle state. If explicit MCP compatibility tools are already available, equivalent `omq_state` calls are optional compatibility, not the default.

- **On start**:
  `omq state write --input '{"mode":"ultraqa","active":true,"current_phase":"planning","iteration":1,"started_at":"<now>","scenario_matrix":[]}' --json`
- **On each cycle**:
  `omq state write --input '{"mode":"ultraqa","current_phase":"qa","iteration":<cycle>,"scenario_matrix":"<updated matrix path or summary>"}' --json`
- **On adversarial e2e transition**:
  `omq state write --input '{"mode":"ultraqa","current_phase":"adversarial-e2e"}' --json`
- **On diagnose/fix transitions**:
  `omq state write --input '{"mode":"ultraqa","current_phase":"diagnose"}' --json`
  `omq state write --input '{"mode":"ultraqa","current_phase":"fix"}' --json`
- **On cleanup transition**:
  `omq state write --input '{"mode":"ultraqa","current_phase":"cleanup"}' --json`
- **On completion**:
  `omq state write --input '{"mode":"ultraqa","active":false,"current_phase":"complete","completed_at":"<now>"}' --json`
- **For resume detection**:
  `omq state read --input '{"mode":"ultraqa"}' --json`
...
`omq state clear --input '{"mode":"ultraqa"}' --json`
